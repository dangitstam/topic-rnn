...
def train_epoch(model: TopicRNN,
    ...
    train_generator = tqdm(iterator(train_dataset, num_epochs=1, cuda_device=cuda_device))
    for batch_iteration, batch in enumerate(train_generator):

        # Shape(s): (batch, max_sequence_length), (batch,)
        input_tokens = batch['input_tokens']
        sentiment = batch['sentiment']
        output_dict, _ = model.forward(input_tokens,
                                                  input_tokens,
                                                  sentiment=sentiment)
...
def evaluate(model: TopicRNN,
    ...
    for batch in evaluation_generator:
        ...
        input_tokens = batch['input_tokens']
        sentiment = batch['sentiment']
        model.forward(input_tokens, input_tokens, sentiment=sentiment)
        metrics = model.get_metrics()
